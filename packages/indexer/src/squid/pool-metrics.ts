...
export const omnipoolYieldMetricsQuery = (squidSdk: SquidSdk) => queryOptions({
    queryKey: [
    ...
    queryFn: async () => {
      ...
      return data.omnipoolAssetsYieldMetrics.nodes.filter(node =>node !== null).map(node => ({
...
export const stablepoolYieldMetricsQuery = (squidSdk: SquidSdk) => queryOptions({
  queryKey: [
