import {
  isH160Address,
  isSS58Address,
  safeConvertAddressH160,
  safeConvertAddressSS58,
} from "@galacticcouncil/utils"
import { standardSchemaResolver } from "@hookform/resolvers/standard-schema"
import { useForm } from "react-hook-form"
import { type infer as Infer, object, string } from "zod"

export type ExternalWalletFormValues = Infer<typeof schema>

const validateH160Address = string().refine((value) =>
  isH160Address(safeConvertAddressH160(value)),
)

const validateSS58Address = string().refine((value) =>
  isSS58Address(safeConvertAddressSS58(value)),
)

const validadateAddress = string()
  .trim()
  .min(1, "Address is required")
  .refine(
    (value) =>
      validateSS58Address.safeParse(value).success ||
      validateH160Address.safeParse(value).success,
    "Invalid address format",
  )

const schema = object({
  address: validadateAddress,
})

const defaultValues: ExternalWalletFormValues = {
  address: "",
}

export const useExternalWalletForm = () => {
  return useForm<ExternalWalletFormValues>({
    defaultValues,
    resolver: standardSchemaResolver(schema),
    mode: "onChange",
  })
}
