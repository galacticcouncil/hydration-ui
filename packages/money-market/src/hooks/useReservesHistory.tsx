...
import { getAdjustedTimestamp, toUnixTimestamp } from "@/utils/date"
...
const resolutionForTimeRange = (
  ...
  switch (timeRange) {
    case ESupportedTimeRanges.OneMonth:
      return {
        from: toUnixTimestamp(getAdjustedTimestamp(now, 30, "day")),
        resolutionInHours: 6,
      }
    case ESupportedTimeRanges.SixMonths:
      return {
        from: toUnixTimestamp(getAdjustedTimestamp(now, 6, "month")),
        resolutionInHours: 24,
      }
    case ESupportedTimeRanges.OneYear:
      return {
        from: toUnixTimestamp(getAdjustedTimestamp(now, 1, "year")),
        resolutionInHours: 24,
      }
    default:
      return {
        from: toUnixTimestamp(now),
        resolutionInHours: 6,
      }
  }
}
