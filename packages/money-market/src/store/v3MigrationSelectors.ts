...
export const selectSplittedBorrowsForMigration = (
  userReserves: ComputedUserReserveData[],
) => {
...
export const selectDefinitiveSupplyAssetForMigration = (
  ...
  const nonIsolatedAssets = store.selectedMigrationSupplyAssets.filter(
    (supplyAsset) => {
      const underlyingAssetAddress =
        selectMigrationUnderlyingAssetWithExceptions(store, supplyAsset)
  ...
  const isolatedAssets = store.selectedMigrationSupplyAssets.filter(
    (supplyAsset) => {
      const underlyingAssetAddress =
        selectMigrationUnderlyingAssetWithExceptions(store, supplyAsset)
      const v3UserReserve = userReservesV3Map[underlyingAssetAddress]
...
export const selectMigrationUnderlyingAssetWithExceptions = (
  ...
  if (
    ...
    store.migrationExceptions[defaultUnderlyingAsset]
  ) {
...
export type IsolatedReserve = FormatReserveUSDResponse & {
  enteringIsolationMode?: boolean
}
export const selectUserReservesForMigration = (
  store: RootStore,
  timestamp: number,
) => {
  const { userReservesData: userReserveV3Data, ...v3ReservesUserSummary } =
    selectV3UserSummary(store, timestamp)
  ...
  if (v3ReservesUserSummary.totalCollateralMarketReferenceCurrency === "0") {
    ...
    if (definitiveAssets.length > 0) {
      const underlyingAssetAddress =
        selectMigrationUnderlyingAssetWithExceptions(store, definitiveAssets[0])
      ...
      if (
        ...
        isolatedReserveV3 = {
          ...definitiveAsset.reserve,
          enteringIsolationMode: true,
        }
      }
    }
  }
  ...
  const mappedSupplyReserves = supplyReserves.map((userReserve) => {
    ...
    if (v3SupplyAsset) {
      const availableSupplies = valueToBigNumber(
        v3SupplyAsset.reserve.supplyCap,
      ).minus(v3SupplyAsset.reserve.totalLiquidity)
    ...
    if (isolatedReserveV3) {
      ...
      if (v3SupplyAsset?.underlyingBalance !== "0") {
        usageAsCollateralEnabledOnUserV3 =
          v3SupplyAsset?.usageAsCollateralEnabledOnUser
      } else {
  ...
  const mappedBorrowReserves = borrowReserves.map((userReserve) => {
    ...
    if (v3BorrowAsset) {
      let liquidationThreshold =
        v3BorrowAsset.reserve.formattedReserveLiquidationThreshold

      if (
        userEmodeCategoryId !== 0 &&
        selectedReserve?.eModeCategoryId !== userEmodeCategoryId
      ) {
        disabledForMigration = MigrationDisabled.EModeBorrowDisabled
        liquidationThreshold =
          v3BorrowAsset.reserve.formattedEModeLiquidationThreshold
      }
...
export const selectedUserSupplyReservesForMigration = (
  ...
  const { supplyReserves, isolatedReserveV3 } = selectUserReservesForMigration(
    store,
    timestamp,
  )
  const selectedUserReserves = supplyReserves.filter(
    (userReserve) =>
      selectMigrationSelectedSupplyIndex(store, userReserve.underlyingAsset) >=
      0,
  )
...
export const selectUserSupplyIncreasedReservesForMigrationPermits = (
  ...
  return selectedUserSupplyReservesForMigration(store, timestamp).map(
    (userReserve) => {
      const increasedAmount = addPercent(userReserve.underlyingBalance)
      const valueInWei = valueToWei(
        increasedAmount,
        userReserve.reserve.decimals,
      )
...
export const selectUserSupplyAssetsForMigrationNoPermit = (
  ...
  const selectedUserSupplyReserves =
    selectUserSupplyIncreasedReservesForMigrationPermits(store, timestamp)
  return selectedUserSupplyReserves.map(
...
export const selectMigrationRepayAssets = (
  ...
  const deadline = Math.floor(Date.now() / 1000 + 3600)
  return selectSelectedBorrowReservesForMigration(store, timestamp).map(
    (userReserve) => ({
...
export const selectMigrationSignedPermits = (
  ...
  const approvalsWithSignatures = store.approvalPermitsForMigrationAssets.map(
    (approval, index) => {
      return {
        ...approval,
...
export const selectSelectedBorrowReservesForMigrationV3 = (
  ...
  const { userReservesData: userReservesDataV3 } = selectV3UserSummary(
    store,
    timestamp,
  )
...
export const selectFormatUserSummaryForMigration = (
  ...
  const { marketReferenceCurrencyDecimals, marketReferenceCurrencyPriceInUsd } =
    baseCurrencyData
...
export const selectV2UserSummaryAfterMigration = (
  store: RootStore,
  currentTimestamp: number,
) => {
...
export const selectMigrationBorrowPermitPayloads = (
  ...
  selectedUserReserves
    ...
    .forEach((userReserve) => {
      const borrowReserveV3 = userReservesDataV3.find(
        (v3Reserve) =>
          v3Reserve.underlyingAsset === userReserve.underlyingAsset,
      )
      if (!borrowReserveV3) {
        return
      }
      ...
      if (!reserveDebts[userReserve.underlyingAsset]) {
        reserveDebts[userReserve.underlyingAsset] = {
          variableDebtTokenAddress:
            borrowReserveV3.reserve.variableDebtTokenAddress,
  ...
  return Object.keys(reserveDebts).map<Approval>((key) => {
    ...
    const totalDebt = valueToBigNumber(debt.stableDebtAmount).plus(
      debt.variableDebtAmount,
    )
    const combinedAmountInWei = valueToWei(totalDebt.toString(), debt.decimals)
    let bufferedAmount = combinedAmountInWei
    if (buffer) {
      const amountBN = valueToBigNumber(bufferedAmount)
...
export const selectV3UserSummaryAfterMigration = (
  ...
  const userReserves = poolReserveV3Summary.userReservesData.map(
    (userReserveData) => {
      const stableBorrowAsset =
        borrowsMap[userReserveData.reserve.stableDebtTokenAddress]
      const variableBorrowAsset =
        borrowsMap[userReserveData.reserve.variableDebtTokenAddress]

      const supplyUnderlyingAssetAddress =
      ...
      let usageAsCollateralEnabledOnUser =
        userReserveData.usageAsCollateralEnabledOnUser
      const variableBorrowIndexV3 = valueToBigNumber(
        userReserveData.reserve.variableBorrowIndex,
      )

      if (
        variableBorrowAsset &&
        variableBorrowAsset.migrationDisabled === undefined
      ) {
        ...
        combinedScaledDownVariableDebtV3 = valueToBigNumber(
          combinedScaledDownVariableDebtV3,
        )
      ...
      if (
        stableBorrowAsset &&
        stableBorrowAsset.migrationDisabled === undefined
      ) {
        const scaledDownStableDebtV2Balance = rayDiv(
          valueToWei(
            stableBorrowAsset.increasedStableBorrows,
            userReserveData.reserve.decimals,
          ),
          variableBorrowIndexV3,
        )
      ...
      if (supplyAsset) {
        usageAsCollateralEnabledOnUser =
          supplyAsset.usageAsCollateralEnabledOnUserV3
        const scaledDownATokenBalance = valueToBigNumber(
          userReserveData.scaledATokenBalance,
        )
        const liquidityIndexV3 = valueToBigNumber(
          userReserveData.reserve.liquidityIndex,
        )
        const underlyingBalanceV2 =
          store.migrationExceptions[supplyUnderlyingAssetAddress]?.amount ||
          valueToWei(
            supplyAsset.underlyingBalance,
            userReserveData.reserve.decimals,
          )
        ...
        combinedScaledDownABalance = scaledDownATokenBalance
          .plus(scaledDownBalanceV2)
          .toString()
      }
...
export type MigrationReserve = ComputedUserReserveData & {
  migrationDisabled?: MigrationDisabled
}
...
export const assetEnabled = (
  selected: MigrationSelectedReserve,
  reserves: MigrationReserve[],
) => {
  const selectedReserve = reserves.find(
    (reserve: MigrationReserve) =>
      selected.underlyingAsset === reserve.underlyingAsset,
  )
  return (
    selectedReserve !== undefined &&
    selectedReserve.migrationDisabled === undefined
  )
}
...
export const computeSelections = (
  ...
  const enabledReserves = reserves.filter(
    (reserve) => reserve.migrationDisabled === undefined,
  )
