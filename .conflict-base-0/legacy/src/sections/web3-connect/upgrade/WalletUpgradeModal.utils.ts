...
function isInjectedExtension(extension: any): extension is InjectedExtension {
  return Boolean(extension?.metadata && extension?.name && extension?.version)
}
...
export const useUpdateMetadataMutation = () => {
  ...
  const { wallet } = getWalletProviderByType(account?.provider)
  const extension = wallet?.extension
  const isInjected = isInjectedExtension(extension)

  const enabled = !!account && !account.isExternalWalletConnected && isInjected

  const state = useQuery(
    QUERY_KEYS.metadataVersion,
    async () => {
      if (!isInjected) throw new Error("Missing extension")
