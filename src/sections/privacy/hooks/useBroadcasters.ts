...
//
// Thin facade over BroadcasterContext that matches the surface the spec
// asked for: `{ list, refresh, selected, select, isMock, status,
// fallbackReason }`. Components that only need to read/select stay
// decoupled from the broadcast() thunk in useBroadcastTransaction.
...
export type UseBroadcasters = {
  ...
  /** Empty-state copy when `list` is empty (or all mock). */
