...
import { Proposal } from "sections/lending/hooks/governance/useProposals"

import {
  isDifferentialReached,
  isQuorumReached,
} from "sections/lending/modules/governance/helpers"
...
export const getProposalVoteInfo = (proposal: Proposal): ProposalVoteInfo => {
  ...
  const currentDifferential = normalizeBN(
    forVotesBN.minus(againstVotesBN),
    18,
  ).toString()
  ...
  const allVotes = new BigNumber(proposal.votes.forVotes).plus(
    proposal.votes.againstVotes,
  )
