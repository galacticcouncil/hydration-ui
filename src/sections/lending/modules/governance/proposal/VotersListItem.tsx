...
import { ProposalVote } from "sections/lending/hooks/governance/useProposalVotes"

import { textCenterEllipsis } from "sections/lending/helpers/text-center-ellipsis"
...
export const VotersListItem = ({
  ...
  const displayName = (name: string) => {
    if (compact) {
      ...
      const compactName =
        name.length <= 10 ? name : textCenterEllipsis(name, 4, 3)
      return compactName
    }
    // Addresses
    if (name === address) {
  ...
  return (
    <Box
      sx={{ my: 6, "&:first-of-type": { mt: 0 }, "&:last-of-type": { mb: 0 } }}
    >
      <Box
        sx={{
          display: "flex",
      ...
      >
        ...
        >
          <Typography
            variant="subheader1"
            color={voter.support ? "success.main" : "error.main"}
          >
