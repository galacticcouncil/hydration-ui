...
import { DelegationTokenType } from "sections/lending/components/transactions/GovDelegation/DelegationTokenSelector"
...
export const useGovernanceDelegate = (
  ...
  const [user, estimateGasLimit] = useRootStore((state) => [
    state.account,
  ...
  const action = async () => {
    if (isSignatureAction) {
      ...
      let txData = await delegationTokenService.batchMetaDelegate(
        user,
        txs,
        connectedChainId,
      )
      ...
      return processTx({
        ...
        successCallback: (txnResponse: TransactionResponse) => {
          ...
          queryClient.invalidateQueries({
            queryKey: queryKeysFactory.powers(
              user,
    ...
    if (delegationTokenType === DelegationTokenType.ALL) {
      ...
      for (const tx of delegationParameters) {
        if (delegationType !== DelegationType.ALL) {
          const payload =
            await delegationTokenService.prepareV3DelegateByTypeSignature(tx)
