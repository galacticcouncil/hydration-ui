...
const filterTokens = (
  ...
  return tokens.filter(
    (token) =>
      token.propositionDelegatee !== "" || token.votingDelegatee !== "",
  )
}
...
export const DelegationTokenSelector = ({
  ...
  const filteredTokens = filter
    ? filterTokens(delegationTokens, delegationType)
    : delegationTokens
  ...
  if (isOneLiner) {
    return (
      <TokenRow
        ...
        amount={filteredTokens[0].amount}
      />
    )
  }
  ...
  return (
    <FormControl variant="standard" fullWidth sx={{ mb: 24 }}>
      <RadioGroup
        ...
        onChange={(e) =>
          setDelegationTokenType(
            Number(e.target.value) as unknown as DelegationTokenType,
          )
        }
      >
        <FormControlLabel
