...
import { SwapVariant } from "sections/lending/hooks/paraswap/common"
import { useCollateralRepaySwap } from "sections/lending/hooks/paraswap/useCollateralRepaySwap"
...
export function CollateralRepayModalContent({
  ...
  const repayAllDebt =
    isMaxSelected &&
    valueToBigNumber(tokenToRepayWithBalance).gte(
  ...
  const handleRepayAmountChange = (value: string) => {
    ...
    if (
      maxSelected &&
      valueToBigNumber(tokenToRepayWithBalance).lt(
        collateralAmountRequiredToCoverDebt,
      )
    ) {
  ...
  const amountAfterRepay = valueToBigNumber(debt).minus(
    BigNumber.min(outputAmount, debt),
  )
  const displayAmountAfterRepayInUsd = amountAfterRepay.multipliedBy(
  ...
  const exactOutputAmount =
    swapVariant === "exactIn" ? outputAmount : repayAmount
  const exactOutputUsd =
    swapVariant === "exactIn" ? outputAmountUSD : repayAmountUsdValue
  ...
  const BlockingError: React.FC = () => {
    switch (blockingError) {
      case ErrorType.NOT_ENOUGH_COLLATERAL_TO_REPAY_WITH:
        return (
          <span>Not enough collateral to repay this amount of debt with</span>
        )
  ...
  return (
    <>
      <AssetInput
        ...
        maxValue={debt}
        inputTitle={<span>Expected amount to repay</span>}
        balanceText={<span>Borrow balance</span>}
      />
      <Box
        sx={{
          padding: "18px",
      ...
      <AssetInput
        ...
        onChange={handleRepayAmountChange}
        inputTitle={<span>Collateral to repay with</span>}
        balanceText={<span>Borrow balance</span>}
      ...
      >
        <DetailsHFLine
          visibleHfChange={
            swapVariant === "exactOut" ? !!amount : !!inputAmount
          }
        ...
        />
        <DetailsNumberLineWithSub
          description={<span>Borrow balance after repay</span>}
        ...
        />
        <DetailsNumberLineWithSub
          description={<span>Collateral balance after repay</span>}
