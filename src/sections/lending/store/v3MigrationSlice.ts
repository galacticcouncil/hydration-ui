...
export type V3MigrationSlice = {
  ...
  toggleMigrationSelectedBorrowAsset: (
    asset: MigrationSelectedBorrowAsset,
  ) => void
  ...
  getMigrationExceptionSupplyBalances: (
    supplies: MigrationSupplyException[],
  ) => void
}
...
> = (set, get) => {
  return {
    ...
    generatePermitPayloadForMigrationSupplyAsset: async ({
      amount,
      underlyingAsset,
      deadline,
    }) => {
    ...
    enforceAsCollateral: (underlyingAsset: string) => {
      set((state) =>
        produce(state, (draft) => {
          ...
          const assetEnforced =
            draft.selectedMigrationSupplyAssets[assetIndex]?.enforced
          if (assetIndex >= 0) {
    ...
    selectAllSupply: (currentTimestamp: number) => {
      const { supplyReserves } = selectUserReservesForMigration(
        get(),
        currentTimestamp,
      )
      if (
        ...
        const nonSelectedSupplies = supplyReserves
          ...
          .filter(
            ({ underlyingAsset }) =>
              selectMigrationSelectedSupplyIndex(get(), underlyingAsset) < 0,
          )
    ...
    selectAllBorrow: (currentTimestamp: number) => {
      const { borrowReserves } = selectUserReservesForMigration(
        get(),
        currentTimestamp,
      )
      if (
    ...
    migrateWithoutPermits: () => {
      ...
      const creditDelegationApprovals: MigrationDelegationApproval[] =
        borrowPermitPayloads.map(({ underlyingAsset, amount }) => ({
          debtTokenAddress: underlyingAsset,
          amount,
        }))
    ...
    migrateWithPermits: async (
      ...
      const { creditDelegationPermits, supplyPermits } =
        selectMigrationSignedPermits(get(), signatures, deadline)
      const supplyAssets = selectUserSupplyAssetsForMigrationNoPermit(
        get(),
        timestamp,
      )
    ...
    getMigrationServiceInstance: () => {
      ...
      const pool = new Pool(provider, {
        ...
        REPAY_WITH_COLLATERAL_ADAPTER:
          currentMarketV3Data.addresses.REPAY_WITH_COLLATERAL_ADAPTER,
        SWAP_COLLATERAL_ADAPTER:
          currentMarketV3Data.addresses.SWAP_COLLATERAL_ADAPTER,
      ...
      const newMigratorInstance = new V3MigrationHelperService(
        provider,
        migratorAddress,
        pool,
      )
    ...
    getMigrationExceptionSupplyBalances: async (supplies) => {
      const chainId =
        get().currentNetworkConfig.underlyingChainId || get().currentChainId
        ...
        if (filteredSuppliesForExceptions.length > 0) {
          ...
          const mappedSupplies = filteredSuppliesForExceptions.map(
            ({ scaledATokenBalance, underlyingAsset }) => {
              return get().getMigrationServiceInstance().getMigrationSupply({
