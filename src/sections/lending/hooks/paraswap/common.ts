...
import {
  GetRateFunctions,
  RateOptions,
} from "@paraswap/sdk/dist/methods/swap/rates"
...
export async function fetchExactInTxParams(
  ...
  const { swapCallData, augustus, destAmountWithSlippage } =
    await swapper.getTransactionParams(
...
export async function fetchExactOutTxParams(
  ...
  const { swapCallData, augustus, srcAmountWithSlippage } =
    await swapper.getTransactionParams(
      swapIn.underlyingAsset,
      swapIn.decimals,
      swapOut.underlyingAsset,
...
export const calculateSignedAmount = (
  ...
  const amountWithMargin =
    Number(amount) + Number(amount) * (margin ?? SIGNATURE_AMOUNT_MARGIN) // 10% margin for aToken interest accrual, custom amount for actions where output amount is variable
