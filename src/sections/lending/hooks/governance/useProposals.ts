...
import {
  getProposalMetadata,
  parseRawIpfs,
} from "sections/lending/modules/governance/utils/getProposalMetadata"
import { VotingMachineService } from "sections/lending/services/VotingMachineService"
import {
  governanceV3Config,
  ipfsGateway,
} from "sections/lending/ui-config/governanceConfig"
...
export const enhanceProposalWithMetadata = async (
  ...
  if (!proposal.proposalMetadata) {
    ...
    const metadata = await parseRawIpfs(
...
export const getProposalsByState = (
  first: number,
  skip: number,
  stateFilter: ProposalV3State,
) =>
  request<{ proposals: SubgraphProposal[] }>(
    governanceV3Config.governanceCoreSubgraphUrl,
...
async function fetchSubgraphProposals(
  pageParam: number,
  proposalStateFilter?: ProposalV3State,
) {
  if (proposalStateFilter) {
    const result = await getProposalsByState(
      PAGE_SIZE,
      pageParam * PAGE_SIZE,
      proposalStateFilter,
    )
...
export async function fetchProposals(
  ...
  const votingMachingData =
    await votingMachineSerivce.getProposalsData(votingMachineParams)
