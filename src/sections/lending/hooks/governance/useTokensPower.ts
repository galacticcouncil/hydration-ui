...
import { FixedPointDecimal } from "sections/lending/architecture/FixedPointDecimal"
import { TokenDelegationPower } from "sections/lending/services/DelegationTokenService"
...
export const useTokensPowers = (tokens: string[]) => {
  ...
  return useQueries({
    queries: tokens.map((token) => ({
      queryFn: () =>
        delegationTokenService.getTokenPowers(
          user,
          token,
          governanceV3Config.coreChainId,
        ),
      queryKey: queryKeysFactory.tokenPowers(
        user,
        token,
        governanceV3Config.coreChainId,
      ),
