  ...
  return useQuery({
    queryFn: () =>
      request<{ voteEmitteds: ProposalVote[] }>(
        governanceV3Config.votingChainConfig[votingChainId as ChainId]
          .subgraphUrl,
        getProposalVotes,
        {
          proposalId,
...
export const useProposalVotes = ({
  ...
  const { data, isFetching } = useProposalVotesQuery({
    proposalId,
    votingChainId,
  })
  ...
  return {
    yaeVotes:
      data?.filter((vote) => vote.support === true).sort(sortByVotingPower) ||
      [],
    nayVotes:
      data?.filter((vote) => vote.support === false).sort(sortByVotingPower) ||
      [],
