...
export const useTokenDelegatees = (tokens: string[]) => {
  ...
  return useQueries({
    queries: tokens.map((token) => ({
      queryFn: () =>
        delegationTokenService.getTokenDelegatees(
          user,
          token,
          governanceV3Config.coreChainId,
        ),
      queryKey: queryKeysFactory.tokenDelegatees(
        user,
        token,
        governanceV3Config.coreChainId,
      ),
...
export const useGovernanceDelegatees = () => {
  const queries = useTokenDelegatees(
    Object.values(governanceV3Config.votingAssets),
  )
