...
} from "sections/lending/modules/history/types"
import {
  USER_TRANSACTIONS_V2,
  USER_TRANSACTIONS_V2_WITH_POOL,
} from "sections/lending/modules/history/v2-user-history-query"
import { USER_TRANSACTIONS_V3 } from "sections/lending/modules/history/v3-user-history-query"
...
export const applyTxHistoryFilters = ({
  ...
  if (searchQuery.length > 0) {
    ...
    filteredTxns = txns.filter((txn: TransactionHistoryItemUnion) => {
      ...
      const altName =
        name.includes("ether") && !name.includes("tether") ? "ethereum" : ""
...
export const useTransactionHistory = ({
  ...
  if (
    ...
    selectedPool =
      currentMarketData.addresses.LENDING_POOL_ADDRESS_PROVIDER.toLowerCase()
  }
  ...
  const {
    ...
    isError,
    error,
  }: UseInfiniteQueryResult<TransactionHistoryItemUnion[], Error> =
    useInfiniteQuery(
