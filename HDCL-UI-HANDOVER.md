...
Companion to `aave-v3-deploy/BIL-MAINNET-HANDOVER.md`. That doc is the
...
- Codebase: `galacticcouncil/hydration-ui`, working branch **`feat/bil-lark2`** (cut from `origin/feat/bil`). The merged BIL Vault commit is `d9a18172c`.
- All BIL UI lives at `apps/main/src/modules/strategies/bil/`. Routed at `/strategies/bil-vault`.
- Vault contracts: **lark-2** (`2.lark.hydration.cloud`, chain `222222`). See `aave-v3-deploy/bil-vault/deployments/lark-2.md` for the canonical address manifest.
- **Mode: vault-only.** The lark-2 contracts redeploy is vault-only — no BIL Aave pool, no deposit-zap, no aToken yet. The `BIL_HAS_AAVE_LAYER` toggle in `constants.ts` is `false`; deposit and redeem hooks fall back to direct vault calls. See "Vault-only mode" below.
...
- **i18n done** — `apps/main/src/i18n/locales/en/bil.json` covers all user-visible strings, `bil` namespace registered, every component uses `useTranslation("bil")`.
- **Live LTVs done** — `useBilReserveConfig` decodes `getConfiguration` bitmap (bits 0-15 LTV, 16-31 liqThreshold) from the BIL pool; `STRATEGY` config remains as a first-paint fallback only. Inert in vault-only mode.
...
|---|---|---|
| Borrow HOLLAR | No BIL Aave pool deployed on lark-2 yet | `BorrowHollarModal` + `useBorrowHollar` (still rendered; `useBilPoolPosition` short-circuits when flag off) |
| Recovery supply (raw BIL → aBIL) | No BIL Aave pool deployed on lark-2 yet | `useSupplyRawBil` throws "Aave layer not deployed" if called |
| Instant redeem (stableswap path) | Depends on aBIL substrate-side asset, which the Aave pool mints | `WithdrawModal` `instantAvailable` prop ties to `BIL_HAS_AAVE_LAYER`; submit button stays disabled |
...
| Live reserve-config LTV decode | Needs the Aave pool's `getConfiguration` view | `useBilReserveConfig` short-circuits when flag off — `STRATEGY` static fallback shown |
...
| Feature flag for BIL nav | Always-on; should gate behind env var until mainnet governance lands | nav config |
...
```
apps/main/src/modules/strategies/bil/
├── BilVaultPage.tsx           # Top-level page, wires everything
├── BilVault.styled.ts         # Legacy styled-components (some pre-existing TS errors)
├── constants.ts                # Addresses, ABIs, STRATEGY config (LTV fallbacks + explorer URL), RPC client
├── assets/
│   ├── bil-logo.svg           # BIL token logo (pink rings)
...
│   ├── MyPositionsTable.tsx    # Inline aBIL row + conditional uncollateralised row
...
│   ├── BilLogo.tsx            # SVG wrapper, size prop
...
│   │                           # useSupplyRawBil (recovery), useRequestRedeemRaw
│   ├── useBilPoolPosition.ts  # useBilPoolPosition (user account data) +
│   │                           # useBilReserveConfig (decodes LTV/liqThreshold bitmap)
│   ├── useBilPoolWrites.ts    # useBorrowHollar
...
Routes:
- `apps/main/src/routes/bil/route.tsx` + `index.tsx` — TanStack Router file-based; mounts `BilVaultPage` under `SubpageLayout`.
- Top-nav config: `apps/main/src/config/navigation.ts` (added `bilVault` entry).

i18n:
- `apps/main/src/i18n/locales/en/bil.json` — namespace covering every
  user-facing string. Registered in `apps/main/src/i18n/index.ts`.
- All components import `useTranslation("bil")` and use `t("key.path")`
...
- **Data fetching**: TanStack Query via `useQuery`. Query keys: `["bil-vault-<scope>", ...keys]`. `refetchInterval` 15-30s.
...
All in `apps/main/src/modules/strategies/bil/constants.ts`. The reference
deployment manifest lives at `aave-v3-deploy/bil-vault/deployments/lark-2.md`
...
`BIL_HAS_AAVE_LAYER` flag.

| Constant | lark-2 address | Notes |
|---|---|---|
| `VAULT_ADDRESS` | `0xbDAFEB…3502` | BIL Vault proxy (ERC-4626 + ERC-7540) |
...
| `BIL_POOL_ADDRESS` | `0x0…0000` | **Not deployed on lark-2 yet** |
| `BIL_ATOKEN_ADDRESS` | `0x0…0000` | **Not deployed on lark-2 yet** |
| `BIL_DEPOSIT_ZAP_ADDRESS` | `0x0…0000` | **Not deployed on lark-2 yet** |
| `BIL_PRECOMPILE_ADDRESS` | `0x000…01000000037` | Substrate-asset alias — stable across deploys |
| `VAULT_DEPLOY_BLOCK` | `138433n` | `fromBlock` for getLogs queries on lark-2 |
| `BIL_HAS_AAVE_LAYER` | `false` | Master toggle for the money-market layer |

The `STRATEGY` static config in `constants.ts` carries display-only metadata (max LTV 80%, liq LTV 90%, About copy, breadcrumb path, asset labels). The on-chain reserve-config decode in `useBilReserveConfig` replaces these on resolve — but only when `BIL_HAS_AAVE_LAYER` is true.

### Vault-only mode (lark-2)

When `BIL_HAS_AAVE_LAYER = false`:
...
- **Borrow / repay / supply-as-collateral** flows are unavailable. `BorrowHollarModal`, `AvailableToBorrowCard`, `MyBorrowsCard`, and the secondary "uncollateralised raw BIL" recovery row stay rendered (component code unchanged) but the hooks behind them (`useBilPoolPosition`, `useBilReserveConfig`, `useBorrowHollar`, etc.) short-circuit when the flag is off.
- **Instant redeem** (stableswap path) is gated off — `instantAvailable={BIL_HAS_AAVE_LAYER}` on `WithdrawModal`. The Instant card stays visible but the submit button stays disabled.
...
|---|---|---|
| `deposit` | `(hollarAmount) → bilMinted` | `(assets, receiver) → shares` |
| `requestRedeem` | `(bilAmount) → requestId` | `(shares, controller, owner) → requestId` |
| `getRedemptionRequest` | 4-tuple `(user, bilAmount, bilFulfilled, active)` | 5-tuple `(user, bilAmount, bilSettled, hollarOwed, active)` |
...
|---|---|---|
| Phase 0 | PR reconciliation | ✅ resolved — keep as standalone module at `/bil-vault`, don't merge into PR #3579's Multiply module |
...
| Phase 4 | BIL Aave pool integration (path C — direct viem reads against pool, no nested MoneyMarketProvider) | ✅ |
| Phase 5 | Instant-redeem stableswap path | ✅ done on lark — pool live, `useStableswap.ts` hooks (`useInstantQuote` + `useInstantRedeem`) wired into `WithdrawModal` + `BilVaultPage`. Both the modal's Instant card and the per-row Instant button now route through the new pool. See "Phase 5 implementation" below for the file-by-file summary and the unit-conversion gotcha that bit me. |
| Phase 6 | i18n migration (`bil.json` namespace + `t('bil.…')` everywhere) | ✅ done — namespace registered, all components migrated, `STRATEGY` constant trimmed to non-display values only |
...
| Phase 8 | Routing reconciliation | ✅ decided not to do; BIL stays standalone |
...
Done on lark for the modal-input path (swap liquid aBIL → HOLLAR). The
...
- [constants.ts](apps/main/src/modules/bil/constants.ts) — added:
  ```ts
  export const STABLESWAP_POOL_ID = 10055n
  // Lark uses 550 for the aToken (named "aBIL"); mainnet target is 55
  // (renamed "BIL" in the launch proposal). TODO Phase 9: switch.
  export const STABLESWAP_BIL_ASSET_ID = 550n
  export const HOLLAR_ASSET_ID = 222n
  ```
- [hooks/useStableswap.ts](apps/main/src/modules/bil/hooks/useStableswap.ts) (new):
  - `useInstantQuote(bilAmount, queueHollarOut)` — debounced 250ms,
    ...
    asset 550 (aBIL) on lark.
  - `useInstantRedeem(bilAmount)` — modal-input flow. Swaps the
    user's LIQUID aBIL balance via `sdk.tx.trade(swap).withSlippage(swapSlippage).withBeneficiary(address).build()`,
...
- [WithdrawMethodPicker.tsx](apps/main/src/modules/bil/components/WithdrawMethodPicker.tsx) — exported `projectRate` so the modal can compute the queue-projected payout. The Instant card detail rows show a concrete payout comparison ("You receive now: X HOLLAR" / "Via queue (~N days): Y HOLLAR" / "Difference: ±Z HOLLAR (P%)") rather than a bare percentage discount — easier to reason about real numbers.
- [WithdrawModal.tsx](apps/main/src/modules/bil/components/WithdrawModal.tsx) — dropped the `instantQuote` prop; modal now owns the quote internally via `useInstantQuote(inputNum, queueHollarOut)`.
- [BilVaultPage.tsx](apps/main/src/modules/bil/BilVaultPage.tsx):
...
|---|---|---|
| User-held aToken | **asset 550** ("aBIL") | **asset 55** ("BIL" after rename) |
| Vault underlying | asset 55 ("BIL") | asset 550 ("BIL") |

Lark pool pairs `[222 HOLLAR, 550 aBIL]`; mainnet will pair `[55 BIL, 222 HOLLAR]`.
Mainnet bring-up: change `STABLESWAP_BIL_ASSET_ID = 550n` → `55n` in
...
I initially passed `parseUnits(bilAmount, 18).toString()` (wei) to
...
Without being on this list, the precompile enforces ERC20 allowance — and the standard money-market UI hardcodes `requiresApproval = false` because the main money-market pool *is* on the list. The BIL pool needed to be added too. `EVMAccounts.approve_contract(Pool-Proxy-BIL)` is now part of the governance proposal (lesson 8 in the on-chain handover).
...
Round-tripping a balance through `Number → toString → parseUnits` can produce a wei value slightly larger than the actual on-chain balance, which causes `transferFrom` to revert with insufficient balance. **Always read bigints fresh inside mutation functions** when accuracy matters. See `useSupplyRawBil` for the pattern.
...
papi defaults to chainHead/v2; the lark RPC will fail with `WS halt` errors. Use `withPolkadotSdkCompat(getWsProvider(rpc))` to shim the legacy methods. See `scripts/encode-bil-asset-update.mjs` for the working pattern (or `scripts/encode-bil-asset-update-pjs.mjs` for the @polkadot/api alternative if papi is fussy).

### 10. Lark doesn't allow `gh pr checkout` of remote branches by default
The clone has remote tracking but local branch creation needs `git checkout -b feat/bil-vault origin/feat/bil-vault` explicitly. `gh pr checkout 3606` failed; manual checkout works.

### 11. `@galacticcouncil/sdk-next` only sees ONE Aave instance — can't trade raw BIL
The SDK's router auto-discovers Aave reserves by calling the `AaveTradeExecutor.pools()` runtime API. On Hydration that API is backed by a single-pool storage value (`pallet_liquidation::BorrowingContract`) which holds *one* EVM Pool address (the main money-market by default). The BIL Aave pool is a SECOND, separate Aave deployment — so its reserves never show up in `pools()`, and the SDK rejects asset 55 (raw BIL on lark) with `"55 is not supported asset"`. Asset 550 (aBIL) works, because the main MM happens to have an aToken with that id registered through unrelated paths.

**What this blocked**: a per-row "Instant redeem" button on `MyWithdrawals` that would atomically `vault.cancelRedeem + sdk.tx.trade(raw BIL → HOLLAR)`. The raw-BIL leg is needed for atomicity (otherwise a silently-failed cancel still lets the trade succeed against the user's *liquid* aBIL — see lesson 1) — but it can't be quoted, so the whole flow is parked.

**What still works**: the modal-input instant redeem (swap liquid aBIL → HOLLAR) — that uses asset 550, which the SDK does see. Users wanting an instant exit on a queued request click Cancel first (`useCancelRedeem` already returns and re-supplies the freed BIL as aBIL), then use the modal's instant path on the freed balance.
...
The user's lark wallet has ~0.000436 BIL in `vault.balanceOf` from pre-zap failed batches. Below `minDisplayBalance` (= `vaultStats.minRedeem`, typically 1 BIL), so the uncollateralised row stays hidden. The recovery `Deposit` button on that row sweeps it whenever shown.
...
3. **Strip / gate diagnostic console.logs** — `useRedemptionQueue` and
   `useRedemptionHistory` have `console.log("[bil-vault] queue scan", …)`
...
5. **Feature flag the nav entry** — until mainnet governance executes, the
   BIL section shouldn't be in production. Gate `navigation.ts` entry behind
   `ENV.VITE_ENV === "testnet" || env.BIL_ENABLED`.
...
- **`aave-v3-deploy/BIL-MAINNET-HANDOVER.md`** — chain-side runbook. Mandatory companion read.
- **`HYDRATION_DESIGN_SYSTEM.md`** (in workspace root) — design tokens, component conventions, color/typography scales.
- **`BIL-vault-specification.md`** (in workspace root) — vault contract spec.
...
- Branch: `feat/bil-vault` (PR #3606)
- Uncommitted: substantial modifications + new files across
  `apps/main/src/modules/bil/*`, plus
  - `apps/main/src/i18n/index.ts` — registers `bil` namespace
  - `apps/main/src/i18n/locales/en/bil.json` — new namespace file
  - `apps/main/src/vite-env.d.ts` — added `vite-plugin-svgr/client` reference
  - `apps/main/src/routes/bil/*` — TanStack Router mount
  - `apps/main/src/config/navigation.ts` — top-nav entry
  - `scripts/encode-bil-asset-update*.mjs` — substrate-call encoders
  - `BIL-UI-HANDOVER.md` — this document
...
- Uncommitted: new + modified files
  - `contracts/BILDepositZap.sol`
  - `tasks/misc/deploy-BILDepositZap.ts`
  - Updates to `BIL-MAINNET-HANDOVER.md` (zap, lesson 9, addresses,
    verification, AND the new "Mainnet single-batch launch composition"
    section covering the stablepool + Treasury bootstrap)
- **To be written**: `tasks/proposals/bil-mainnet-launch.ts` —
  the single-batch mainnet proposal task. Mirrors `heurc-launch.ts`
  structure. Specs are fully captured in BIL-MAINNET-HANDOVER.md.
