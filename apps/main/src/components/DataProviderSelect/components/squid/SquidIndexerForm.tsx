...
import {
  SquidIndexerFormValues,
  useSquidIndexerFormSchema,
} from "@/components/DataProviderSelect/components/squid/SquidIndexerForm.utils"
import { useSquidListStore } from "@/states/provider"

export const SquidIndexerForm = () => {
  const { t } = useTranslation()
  const { addSquid } = useSquidListStore()

  const form = useForm<SquidIndexerFormValues>({
    defaultValues: { address: "" },
    mode: "onChange",
    resolver: standardSchemaResolver(useSquidIndexerFormSchema()),
  })

  const { mutate, isPending } = useMutation({
    mutationFn: async ({ address }: SquidIndexerFormValues) => {
      try {
