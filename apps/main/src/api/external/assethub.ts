...
export const getAssetHubTransferTx = (
  asset: Asset,
  destAddress: string,
  amount: bigint,
) => {
  const location = assethub.getAssetXcmLocation(asset)
  const api = assethub.client.getTypedApi(hub)

  const isNative = location && location.interior === XcmV3Junctions.Here().type
  ...
  return api.tx.Assets.transfer({
    id: Number(assethub.getAssetId(asset)),
    target: MultiAddress.Id(destAddress),
    amount,
  })
}
