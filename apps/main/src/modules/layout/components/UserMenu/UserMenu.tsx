    ...
    <HoverCard open={open} onOpenChange={onOpenChange} closeDelay={180}>
      ...
      >
        ...
        {connectedTypes.map((type) => {
          ...
          const onItemClick = () => {
            if (isActiveProvider && providerAccounts.length > 1) {
              onOpenChange(false)
              toggle(undefined, { initialProvider: type })
            } else if (!isActiveProvider && storedRecentAccount) {
              setAccount(storedRecentAccount)
            }
          }

          return (
            <MenuSelectionItem key={type} onClick={onItemClick}>
