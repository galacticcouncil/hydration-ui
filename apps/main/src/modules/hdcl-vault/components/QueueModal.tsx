    ...
    <SModalBackdrop onClick={onClose}>
      <SModalContent onClick={(e: React.MouseEvent) => e.stopPropagation()}>
        <Flex justify="space-between" align="center">
          <Text fs="h7" fw={500} color={getToken("text.high")}>Withdrawal Queue</Text>
          <Text
            fs="h7"
        ...
        <STableHeader css={{ gridTemplateColumns: "0.5fr 1.5fr 1fr", gap: 8 }}>
          <Text fs="p5" fw={500} color={getToken("text.low")}>#</Text>
          <Text fs="p5" fw={500} color={getToken("text.low")}>Amount</Text>
          <Text fs="p5" fw={500} color={getToken("text.low")} css={{ textAlign: "right" }}>Est. time</Text>
        </STableHeader>
        ...
        {queue.length === 0 && (
          <Flex justify="center" sx={{ py: "xl" }}>
            <Text fs="p4" color={getToken("text.low")}>Queue is empty</Text>
          </Flex>
        )}
      </SModalContent>
    </SModalBackdrop>
