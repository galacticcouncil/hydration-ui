...
import { bigShift, HYDRATION_CHAIN_KEY } from "@galacticcouncil/utils"
...
export const DepositSuccess: React.FC<DepositSuccessProps> = ({
  ...
  const dstChain = chainsMap.get(HYDRATION_CHAIN_KEY)
  ...
  return (
    <ModalBody>
      <Stack gap="s">
        ...
        {assetDetails && dstChain && (
          <Text fs="p3" lh={1.4} color={getToken("text.low")} align="center">
            <Trans
              t={t}
              i18nKey="onramp:deposit.success.description"
              values={{
                amount: bigShift(amount, -assetDetails.decimals).toString(),
