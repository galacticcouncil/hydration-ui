...
export const MarketSummaryTwap: FC<Props> = ({ swap, twap, isLoading }) => {
  ...
  const calculateTwapPctDiff = (): number => {
    const twapPrice = twap.minAmountOut
    const swapPrice =
      swap.amountOut - calculateSlippage(swap.amountOut, slippage)

    return calculateDiffToRef(twapPrice, swapPrice)
  }

  const twapDiff = calculateTwapPctDiff()
