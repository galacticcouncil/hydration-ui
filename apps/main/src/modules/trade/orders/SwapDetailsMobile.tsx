...
type Props = {
  readonly details: MyRecentActivitySwap
}

export const SwapDetailsMobile = ({ details }: Props) => {
  ...
  return (
    <div>
      ...
      {details.status?.kind === "dca" && details.status.blocksPeriod && (
        <>
          <ModalContentDivider />
          <Box p={getTokenPx("containers.paddings.primary")}>
            <Amount
              value={t("trade:trade.orders.mobileDetail.schedulePeriod", {
                timeframe: formatDistanceToNow(
                  Date.now() +
                    details.status.blocksPeriod * PARACHAIN_BLOCK_TIME,
                ),
                count: details.status.blocksPeriod,
              })}
              label="Date"
            />
          </Box>
