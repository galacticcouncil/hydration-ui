...
export const DepositPanel = ({
  ...
  const { data: previewBil } = usePreviewDeposit(debouncedAmount)
  const totalFeesUsd =
    previewBil !== undefined && debouncedAmount > 0
      ? Math.max(0, debouncedAmount - previewBil * vaultStats.exchangeRate)
      : undefined */
  ...
  const ctaLabel = (() => {
    ...
    if (isBelowMin)
      return t("bil.deposit.cta.belowMin", {
  ...
  const amountError = overBalance
    ? t("bil.withdraw.cta.insufficient")
    : isBelowMin
      ? t("bil.deposit.cta.belowMin", {
          value: vaultStats.minDeposit,
          symbol: hollar.symbol,
        })
      : undefined
