...
export const ClaimRewardsButton = () => {
  ...
  const { claimableAmount, totalAmount } = useLiquidityMiningRewards()
      ...
      >
        <Flex direction="column" gap={getTokenPx("scales.paddings.m")}>
          ...
          <Text fs={16} color={getToken("text.high")} fw={500}>
            {t("common:currency", { value: claimableAmount, symbol: "HDX" })}
          </Text>
          <Separator />
          <Text fs="p5" color={getToken("text.high")} fw={400}>
            Total of ≈ {t("common:currency", { value: totalAmount })}
          </Text>
