...
export const useProcessedVotes = (
  ...
  const voteIds = useMemo(() => {
    ...
    const newProcessedVotesIds = data.newProcessedVotes.map(({ keyArgs }) => {
      const [, id] = keyArgs
      const accoutVote = votes.find((vote) => vote.id === id)
...
export const usePendingVotes = (
  ...
  const { data, isLoading } = useQuery(
    pendingVotesQuery(rpc, address, positionId, votesSuccess),
  )
  ...
  const voteIds = useMemo(() => {
    ...
    const newPendingVotesIds = data.newPendingVotes.map(([position]) => {
      const id = position
      const accoutVote = votes.find((vote) => vote.id === id)
