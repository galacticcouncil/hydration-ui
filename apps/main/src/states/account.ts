...
export const useAccountBalancesWithPriceByAssetType = (
  ...
  const {
    ...
    // Shape whatever is in the store, loading or not. Balances restored from
    // cache are here before the live subscriptions connect, and skipping them
    // while `isBalanceLoading` also left `priceIds` empty — which meant the
    // price subscription below only started *after* balances landed instead of
    // alongside them.
  ...
  return {
    ...
    // on `isBalanceLoading` from the store directly
